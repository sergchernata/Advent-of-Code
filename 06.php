...
toggle 628,958 through 811,992";
    ...
    foreach ($dirs as $d)
    {
        ...
        if($parts[0] == 'turn')
        {
            $actn = $parts[1];// on or off
            $from = explode(",", $parts[2]);
        }
        else// toggle
        {
            $actn = $parts[0];// toggle in this case
            $from = explode(",", $parts[1]);
        }

        $from_x = $from[0];
        $from_y = $from[1];
        $to_x = explode(",", $parts[count($parts) - 1])[0];
        $to_y = explode(",", $parts[count($parts) - 1])[1];

        for ($x=$from_x; $x <= $to_x; $x++)
        {
            for ($y=$from_y; $y <= $to_y; $y++)
            {
                if(!isset($lights[$x])){ $lights[$x] = array(); }
                if(!isset($lights[$x][$y])){ $lights[$x][$y] = 0; }

                switch ($actn) {
                    case 'on':
                        $lights_on = $lights[$x][$y] == 0 ? ++$lights_on : $lights_on;
                        $brightness++;
                        $lights[$x][$y]++;
                        break;
                    ...
                    case 'off':
                        ...
                        $lights[$x][$y] = $lights[$x][$y] > 0 ? --$lights[$x][$y] : 0;
                        $lights_on = $lights[$x][$y] == 0 ? --$lights_on : $lights_on;
                        break;
