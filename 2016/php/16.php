...
Sue 500: cats: 2, goldfish: 9, children: 8";
        ...
        foreach ($aunts as $a)
        {
            $aunt = substr($a, 0, strpos($a, ":"));
            $params = explode(", ", ltrim(strstr($a, ": "), ": "));
            if(!isset($comparison[$aunt])){ $comparison[$aunt] = 0; }

            foreach ($params as $p)
            {
                $p = explode(": ", $p);

                foreach ($clues as $ck => $cv)
                {
                    $match = false;

                    if($ck == 'cats' && $p[0] == 'cats' || $ck == 'trees' && $p[0] == 'trees')// for part 2
                    {
                        $match = $p[1] > $cv;
                    }
                    elseif($ck == 'pomeranians' && $p[0] == 'pomeranians' || $ck == 'goldfish' && $p[0] == 'goldfish')// for part 2
                    {
                        $match = $p[1] < $cv;
                    }
                    else// only this is needed for part 1
                    {
                        $match = $p[0] == $ck && $p[1] == $cv;
                    }
        ...
        $maxs = array_keys($comparison, max($comparison))[0];
        var_dump($maxs);